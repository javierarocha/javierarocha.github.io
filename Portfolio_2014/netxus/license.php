 ...
 */
if ( get_option( 'api_manager_theme_ample_activated' ) != 'Activated' ) {
	add_action( 'admin_notices', 'API_Manager_theme_Ample::am_theme_ample_inactive_notice' );
}

class API_Manager_theme_Ample {
	 ...
	 */
	public $api_manager_theme_ample_version_name = 'api_manager_theme_ample_version';
	...
	public $ame_update_check = 'am_ample_theme_update_check';

	public $api_manager_theme_ample_key;
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'ample' ), '1.2' );
	}
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->ame_data_key 				= 'api_manager_theme_ample';
			$this->ame_api_key 					= 'api_key';
			$this->ame_activation_email 		= 'activation_email';
			$this->ame_product_id_key 			= 'api_manager_theme_ample_product_id';
			$this->ame_instance_key 			= 'api_manager_theme_ample_instance';
			$this->ame_deactivate_checkbox_key 	= 'api_manager_theme_ample_deactivate_checkbox';
			$this->ame_activated_key 			= 'api_manager_theme_ample_activated';

			/**
			 * Set all admin menu data
			 */
			$this->ame_deactivate_checkbox 			= 'am_deactivate_ample_checkbox';
			$this->ame_activation_tab_key 			= 'api_manager_theme_ample_dashboard';
			$this->ame_deactivation_tab_key 		= 'api_manager_theme_ample_deactivation';
			$this->ame_settings_menu_title 			= 'Ample License Manager';
			$this->ame_settings_title 				= 'Ample License Manager';
			$this->ame_menu_tab_activation_title 	= __( 'License Activation', 'ample' );
			$this->ame_menu_tab_deactivation_title 	= __( 'License Deactivation', 'ample' );
			 ...
			 */
			$this->ame_software_product_id = __( $this->ame_product_id, 'ample' );

			// Performs activations and deactivations of API License Keys
			require_once( 'am/classes/class-wc-key-api.php' );
			$this->api_manager_theme_ample_key = new Api_Manager_theme_Ample_Key();
	 ...
	 * Update Check Class.
	 *
	 * @return API_Manager_Ample_theme_Update_API_Check
	 */
	public function update_check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '' ) {
		...
		return API_Manager_Ample_theme_Update_API_Check::instance( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
	}
	...
	public function activation() {
		...
		if ( get_option( $this->ame_data_key ) === false || get_option( $this->ame_instance_key ) === false ) {
			...
			$api_manager_theme_ample_password_management = new API_Manager_Ample_theme_Password_Management();

			// Generate a unique installation $instance id
			$instance = $api_manager_theme_ample_password_management->generate_password( 12, false );
			...
			$curr_ver = get_option( $this->api_manager_theme_ample_version_name );

			// checks if the current plugin version is lower than the version being installed
			if ( version_compare( $this->version, $curr_ver, '>' ) ) {
				// update the version
				update_option( $this->api_manager_theme_ample_version_name, $this->version );
			}
	...
	public function license_key_deactivation() {
		...
		if ( $activation_status == 'Activated' && $api_key != '' && $api_email != '' ) {
			$this->api_manager_theme_ample_key->deactivate( $args ); // reset license key activation
		}
	}
     ...
     */
	public static function am_theme_ample_inactive_notice() { ?>
		<?php if ( ! current_user_can( 'manage_options' ) ) return; ?>
		<?php if ( isset( $_GET['page'] ) && 'api_manager_theme_ample_dashboard' == $_GET['page'] ) return; ?>
		<div id="message" class="error">
			<p><?php printf( __( 'The License Key for Theme Ample has not been activated, %sClick here%s to activate the license key.', 'ample' ), '<a href="' . esc_url( admin_url( 'options-general.php?page=api_manager_theme_ample_dashboard' ) ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'ample' ), $this->ame_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
				<?php
			}
...
function AMET() {
    return API_Manager_theme_Ample::instance();
}
